...
pub async fn run(app: &mut App, socket: &Path) -> Result<()> {
    ...
    loop {
        ...
        // v0.2: drain app.pending_outgoing — messages the app wants
        // to send back to the host (e.g. OpenFile after Enter on a
        // file row). Sent before the next Frame so the host can
        // react in the same render tick.
        let outgoing: Vec<Message> = app.pending_outgoing.drain(..).collect();
